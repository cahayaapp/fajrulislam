  ...
  try {
    ...
    if (category) {
      ...
      for (const studentKey of studentKeys) {
        const indexPath = `cahaya_app/wali_index/${studentKey}/${category}`;
        indexed = window.CahayaBandwidth
          ? await window.CahayaBandwidth.readCompat(database, indexPath, {
              ttl: 30 * 60 * 1000,
              limit: safeLimit,
              noCache: Boolean(options.noCache)
            })
          : await database.ref(indexPath).limitToLast(safeLimit).once("value");
        if (indexed.exists()) break;
      }
      return {ok:true,snapshot:indexed,value:indexed?.val() || {},indexed:true,empty:!indexed?.exists()};
    }
    ...
    return { ok:true, snapshot, value:snapshot.val() };
  } catch (error) {
    console.warn(`Data ${path} belum dapat dibaca:`, error);
    return { ok:false, snapshot:null, value:null, error };
  }
}

async function readSnapshot(database, path, options = {}) {
  const result = await readRTDB(database, path, options);
