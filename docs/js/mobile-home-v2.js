...
(() => {
  ...
  const descriptions={
    ...
    'menu-absen-asrama':'Catat kehadiran santri','menu-jurnal-piket':'Catat jalannya program','menu-gamifikasi':'Apresiasi tindakan positif','menu-naqib-teladan':'Keteladanan diri hari ini','menu-mentoring-individu':'Klarifikasi & target pekanan','menu-mentoring-usrah':'Perkembangan target santri','menu-media':'Tautan untuk wali santri','menu-gallery-dokumentasi':'Arsip folder dokumentasi di Drive','menu-manajemen-konten':'Rencana, produksi, dan evaluasi konten',
    'menu-kontrol-pendidikan':'Pantau pendidikan hari ini','menu-materi-pembelajaran':'Target dan realisasi materi','menu-tindak-akademik':'Tuntaskan temuan pendidikan','menu-absensi-ibadah-guru':'Absensi ibadah Guru Mukim','menu-pengaturan-guru-mukim':'Atur Guru Mukim','menu-absensi-ibadah-naqib':'Absensi teladan ibadah Naqib',
    'menu-dashboard-operasional':'Catat satu sesi layanan makan dan kendalanya','menu-dashboard-operasional--checklist':'Pastikan area dapur bersih dan siap','menu-dapur-stock':'Pantau bahan dan ketersediaan stok','menu-dapur-menu':'Lihat jadwal menu dari Supervisor Dapur','menu-dapur-shopping':'Susun kebutuhan belanja untuk besok','menu-dapur-received':'Cocokkan bahan datang dengan daftar belanja','menu-jurnal-liburan-wali':'Checklist kebiasaan baik dan Bedah Rapor ananda','menu-jurnal-liburan-monitor':'Baca jurnal liburan santri dalam pendampingan',
  };
  ...
  async function init(){
    ...
    if(!wali&&!window.CahayaRoleMenuV2){await new Promise((resolve,reject)=>{const script=document.createElement('script');script.src=new URL('js/role-menu-v2.js?v=274',root);script.onload=resolve;script.onerror=reject;document.head.append(script)})}
    ...
    if(role==='LAYANAN_KEBERSIHAN'){
      const descriptions={'menu-jurnal-pkl':'Catat pelaksanaan tugas hari ini','menu-buku-tamu':'Catat tamu dan pelayanan hari ini','menu-laporan-murojaah':'Laporkan pelanggaran atau kejadian santri','menu-penitipan-barang':'Catat barang titipan hari ini'};
      quick=quick.map((item,index)=>({...item,description:descriptions[item.id],icon:index===0?'report':index===1?'people':index===2?'warning':'grid'}));
    }
    if(role==='DAPUR'){
      const desc={'menu-dashboard-operasional':'Catat satu sesi layanan makan dan kendalanya','menu-dapur-shopping':'Susun kebutuhan belanja untuk besok','menu-dapur-received':'Cocokkan bahan datang dengan daftar belanja','menu-dapur-menu':'Lihat jadwal menu dari Supervisor Dapur'};
      quick=quick.map((item,index)=>({...item,description:desc[item.id],icon:['report','grid','check','book'][index]}));
    }
    if(wali){
      const waliQuick={
