/* CAHAYA APP V68 — Push Notification Portal Wali + Global */
(function(){
  'use strict';
  ...
  function hidePrompt(){document.getElementById('cahayaPushPromptV65')?.remove()}

  function showToast(title,body){
    let toast=document.getElementById('cahayaPushToastV65');
    if(!toast){
      toast=document.createElement('div');toast.id='cahayaPushToastV65';
  ...
  function showPrompt(){
    if(document.getElementById('cahayaPushPromptV65')||Notification.permission!=='default')return;
    const el=document.createElement('div');
    el.id='cahayaPushPromptV65';
  ...
  async function prepare(){
    ...
    state.registration=await navigator.serviceWorker.register('firebase-messaging-sw.js?v=68',{scope:'./'});
  ...
  async function registerToken(){
    ...
    // Simpan jalur lama khusus wali + jalur global seluruh pengguna.
    updates[`cahaya_app/fcm_tokens_user/${waliKey}/${tokenKey}`]={...payload,label:clean(account.label||account.nama||account.namaTampilan||username),roles:['wali']};
    updates[`cahaya_app/fcm_users_by_role/wali/${waliKey}`]={username,label:clean(account.label||account.nama||account.namaTampilan||username),roles:['wali'],aktif:true,updatedAt:payload.updatedAt};
    await db().ref().update(updates);
    localStorage.setItem('cahayaPushRegisteredV68',JSON.stringify({token,tokenKey,waliKey,updatedAt:payload.updatedAt}));
    return token;
  }

  async function deactivateCurrentDevice(){
    const {account}=session();
    const username=usernameOf(account);if(!username)return false;
    const waliKey=safeKey(username);
    let stored={};try{stored=JSON.parse(localStorage.getItem('cahayaPushRegisteredV68')||'{}')||{}}catch(_){stored={}}
    ...
    const tokenKey=stored.tokenKey||safeKey(token.slice(-48));const now=new Date().toISOString();
    const updates={};
    updates[`cahaya_app/fcm_tokens_wali/${waliKey}/${tokenKey}/aktif`]=false;
    updates[`cahaya_app/fcm_tokens_wali/${waliKey}/${tokenKey}/logoutAt`]=now;
    updates[`cahaya_app/fcm_tokens_user/${waliKey}/${tokenKey}/aktif`]=false;
    updates[`cahaya_app/fcm_tokens_user/${waliKey}/${tokenKey}/logoutAt`]=now;
    try{await db().ref().update(updates);localStorage.removeItem('cahayaPushRegisteredV68');return true}catch(err){console.warn('Token wali belum dinonaktifkan:',err);return false}
  }
  ...
  window.CahayaWaliPush={enable,registerToken,deactivateCurrentDevice,showPrompt};
})();
