...
const PATH={schedule:'cahaya_app/jadwal_pelajaran',attendance:'cahaya_app/absensi_pembelajaran',absence:'cahaya_app/guru_berhalangan',targets:'kalender_materi_pembelajaran/ta_2026_2027',legacyTargets:'guru_kits_materi/ta_2026_2027',completion:'cahaya_app/capaian_materi_guru',scores:'cahaya_app/nilai_ujian_bulanan',periodicScores:'cahaya_app/nilai_ujian',final:'cahaya_app/nilai_input_final',scoreIndex:'cahaya_app/nilai_input_index',publication:'cahaya_app/rapor_publication',master:'cahaya_app/master_akademik',findings:'cahaya_app/masalah_pendidikan',coaching:'cahaya_app/pembinaan_guru',observation:'observasi_lapangan'};
...
const config={control:['KONTROL OPERASIONAL','Kontrol Hari Ini','Pantau jadwal, kehadiran Guru, pelaksanaan KBM, pembukaan, finalisasi, kelas kosong, dan isu hari ini.'],material:['CAPAIAN MATERI','Capaian Materi','Bandingkan target dan laporan capaian materi Guru Pondok.'],followup:['TINDAK LANJUT','Tindak Lanjut','Kerjakan temuan aktif dari seluruh sumber operasional pendidikan.'],teachers:['PERSONIL PENDIDIKAN','Guru Pondok','Direktori dan konteks operasional Guru Pondok dalam unit Anda.'],scores:['PROGRES NILAI & REVISI','Progres Input Nilai','Pantau progres Parsial dan Final setiap Guru berdasarkan jadwal aktif, serta teruskan permintaan perbaikan Supervisor.'],observation:['OBSERVASI OPERASIONAL','Observasi Pembelajaran','Catat kekuatan dan area perbaikan pelaksanaan pembelajaran.'],findings:['RIWAYAT OPERASIONAL','Riwayat Temuan','Tinjau temuan yang sudah selesai berdasarkan kalender.'],coaching:['PEMBINAAN PERSONIL','Pembinaan Guru','Catat apresiasi, coaching, briefing ulang, atau target perbaikan.'],kpi:['BUKTI OPERASIONAL','KPI Manajer Pendidikan','Bukti kerja pendidikan pada periode terpilih; bukan skor atau peringkat.'],guide:['PANDUAN KERJA','Panduan Manajer Pendidikan','Batas scope, siklus kontrol, dan jalur eskalasi operasional.']};
let state={schedule:[],rows:[],master:{},findings:[],recentFindings:[],scoreSessions:new Map,reportRequests:new Map,findingReadError:null,busy:false,suspended:false};
...
function stableStudentKey(value=''){return String(value||'').normalize('NFD').replace(/[\u0300-\u036f]/g,'').toLowerCase().replace(/[^a-z0-9]/g,'')}
function sessionClasses(marker){return String(marker.kelasScope||'').split('|').map(s=>s.trim()).filter(Boolean)}
function publicationRequests(root){
  const found=[];
  function visit(node,path){
    if(!node||typeof node!=='object')return;
    if(node.studentKey&&node.reportType&&node.period&&node.tahunAkademik){found.push({...node,_path:path});return}
    Object.entries(node).forEach(([key,value])=>visit(value,`${path}/${key}`));
  }
  visit(root,PATH.publication);
  return found;
}
function sessionHasStudent(session,request){
  const wanted=stableStudentKey(request.studentKey||request.studentName||request.namaSantri);
  return session.records.some((item,index)=>{
    const data=item.data||{};
    return [data.studentKey,data.santriId,data.studentId,data.nama_santri,data.namaSantri,item.studentKey,index].some(value=>stableStudentKey(value)===wanted);
  });
}
function requestSessions(request){
  return [...state.scoreSessions.entries()].filter(([,session])=>{
    const marker=session.marker,classes=sessionClasses(marker),type=norm(request.reportType);
    return norm(marker.jenisUjian)===type&&String(marker.tahunAkademik||'')===String(request.tahunAkademik||'')&&norm(marker.periode)===norm(request.period)&&(!request.kelas||classes.some(name=>norm(name)===norm(request.kelas)))&&sessionHasStudent(session,request);
  });
}
...
async function loadSchedule(){
  ...
  const cacheId=`${cacheKey}:schedule:v2`,cached=sharedCache?.get(cacheId,60000);
  if(cached){state.schedule=cached;return state.schedule}
  // Jadwal Firebase adalah sumber kelas terbaru. JSON/cache lama hanya fallback.
  try{state.schedule=rows(await readOne(PATH.schedule))}
  catch(error){console.warn('Jadwal Firebase belum dapat dibaca; mencoba fallback lokal',error)}
  if(state.schedule.length)state.schedule=window.CahayaAcademicInputProgress.activeSchedule(state.schedule,{unit});
  if(!state.schedule.length){
  try{
    const response=await fetch('../data/jadwal-pelajaran-awal-2026-2027.json?v=275');
  ...
  if(!state.schedule.length)throw new Error('Jadwal Kepondokan belum dapat dimuat. Periksa koneksi lalu coba Segarkan.');
  state.schedule=window.CahayaAcademicInputProgress.activeSchedule(state.schedule,{unit});
...
async function loadScores(){
  setState('Memuat progres nilai dan permintaan perbaikan dalam scope…');await loadSchedule();const type=$('scoreType').value,year=$('scoreYear').value,period=$('scorePeriod').value,sourcePath=type==='bulanan'?PATH.scores:PATH.periodicScores;let scores=[];
  try{scores=education(await read(sourcePath))}catch(error){console.warn('Nilai belum dapat dimuat',error)}
  const teachers=window.CahayaAcademicInputProgress.summarize({schedule:state.schedule,scores,master:window.CAHAYA_MASTER_DATA||{},type,year,period,unit}),expected=teachers.reduce((n,x)=>n+x.expected,0),entered=teachers.reduce((n,x)=>n+x.entered,0),final=teachers.reduce((n,x)=>n+x.final,0),done=teachers.filter(x=>x.expected&&x.final===x.expected).length;
  stat([[teachers.length,'Guru terjadwal'],[expected?Math.round(entered/expected*100)+'%':'0%','Input terisi'],[expected?Math.round(final/expected*100)+'%':'0%','Sudah Final'],[done,'Guru selesai']]);
  $('content').innerHTML=teachers.length?teachers.map(teacher=>{const complete=teacher.expected&&teacher.final===teacher.expected,label=complete?'Selesai':teacher.entered?'Berjalan':'Belum Input',tone=complete?'ok':teacher.entered?'att':'bad',details=teacher.assignments.map(item=>`<div style="display:flex;justify-content:space-between;gap:10px;padding:8px 0;border-top:1px solid #e5edf7"><span><b>${esc(item.subject)}</b><br>${esc(item.classes.join(' + '))}</span><span style="text-align:right"><b>${item.progress}% terisi</b><br>${item.final} Final • ${item.partial} Parsial</span></div>`).join(''),summary=!teacher.entered?`${PLabel(type)} ${period} belum diinput.`:`Input ${PLabel(type)} ${period} belum seluruhnya Final.`,action=complete?'':`<button class="me-btn warn" data-new-finding="${esc(teacher.teacherCode||teacher.id)}" data-source="NILAI_UJIAN" data-source-path="${sourcePath}" data-personnel="${esc(teacher.teacher)}" data-subject="${esc(`${PLabel(type)} • ${period}`)}" data-summary="${esc(summary)}">Buat Temuan</button>`;return rowCard(teacher.teacher,`${teacher.entered}/${teacher.expected} nilai terisi • ${teacher.finalProgress}% Final<br>${details}`,status(label,tone),action)}).join(''):'<div class="me-empty">Jadwal Kepondokan aktif belum tersedia.</div>';
  await loadFinalScoreSessions(type,year,period);await loadReportCorrectionRequests();hideState()
}
function PLabel(type){return type==='triwulan'?'Triwulan':type==='semester'?'Semester':'Bulanan'}
async function loadFinalScoreSessions(type,year,period){
  ...
  for(const [key,marker] of Object.entries(markers)){
    ...
    const records=Object.entries(indexes[key]||{}).map(([studentKey,item])=>({...item,studentKey}));
    if(!scoreSessionScope(marker,records))continue;
    state.scoreSessions.set(key,{marker,records});
    if(norm(marker.jenisUjian)!==norm(PLabel(type))||norm(marker.tahunAkademik)!==norm(year)||norm(marker.periode)!==norm(period))continue;
  ...
  $('content').insertAdjacentHTML('afterbegin',`<div class="me-empty"><b>Sesi ${esc(PLabel(type))} • ${esc(period)}</b><br>Sesi Final dapat dibuka kembali melalui mekanisme revisi.</div>`+(cards.join('')||'<div class="me-empty">Belum ada sesi Final pada periode ini.</div>'));
}
async function loadReportCorrectionRequests(){
  state.reportRequests.clear();
  if(session.activeRole!=='MANAJER'||!R.isEducationManager(a))return;
  let root={};
  try{root=await readOne(PATH.publication)}catch(error){console.warn('Permintaan perbaikan rapor belum dapat dimuat',error);return}
  const requests=publicationRequests(root).filter(item=>item.status==='NEEDS_REVISION'&&unitOf({kelas:item.kelas})===unit&&inScope({kelas:item.kelas})).sort((a,b)=>String(b.managerRequest?.requestedAt||b.reviewedAt||b.updatedAt||'').localeCompare(String(a.managerRequest?.requestedAt||a.reviewedAt||a.updatedAt||'')));
  const cards=requests.map(request=>{
    state.reportRequests.set(request._path,request);
    const sessions=requestSessions(request),workflow=request.managerRequest?.status||'PENDING_MANAGER',forwarded=workflow==='FORWARDED_TO_TEACHER';
    const source=`${esc(request.managerRequest?.requestedByName||request.reviewedByName||'Supervisor Pendidikan')} • ${esc(request.managerRequest?.requestedAt||request.reviewedAt||request.updatedAt||'')}`;
    const note=esc(request.managerRequest?.note||request.reviewNote||'Catatan perbaikan belum tersedia.');
    let actions='';
    if(forwarded){
      actions=`<span class="me-status att">Diteruskan ke ${esc(request.managerRequest?.teacherName||'Guru')}</span>`;
    }else if(sessions.length){
      actions=sessions.map(([key,session])=>{
        const marker=session.marker,label=marker.status==='FINAL'?`Teruskan ke ${marker.guru} • ${marker.mapel}`:`Revisi sudah terbuka • ${marker.guru} • ${marker.mapel}`;
        return `<button type="button" class="me-btn ${marker.status==='FINAL'?'primary':''}" data-forward-report="${esc(request._path)}" data-score-session="${esc(key)}" ${marker.status==='FINAL'?'':'disabled'}>${esc(label)}</button>`;
      }).join('');
    }else actions='<span class="me-status bad">Sesi nilai terkait belum ditemukan</span>';
    return rowCard(`${request.studentName||request.namaSantri||'Santri'} • ${request.kelas||'Kelas'}`,`${esc(request.reportType||'Laporan')} • ${esc(request.period)} • ${esc(request.tahunAkademik)}<br><b>Catatan Supervisor:</b> ${note}<br>${source}`,status(forwarded?'Sudah Diteruskan':'Menunggu Manajer',forwarded?'att':'bad'),actions);
  });
  $('content').insertAdjacentHTML('afterbegin',`<div class="me-empty"><b>Permintaan Perbaikan dari Supervisor</b><br>Manajer memilih sesi yang tepat, lalu meneruskan instruksi dan membuka revisi untuk Guru pengampu.</div>`+(cards.join('')||'<div class="me-empty">Tidak ada permintaan perbaikan dari Supervisor.</div>'));
}
function teacherDirectory(){const map=new Map;state.schedule.forEach(s=>{const key=norm(s.guruKode||s.guruNama||s.guru);if(!key)return;if(!map.has(key))map.set(key,{name:s.guruNama||s.guru||s.guruKode,subjects:new Set,classes:new Set});const t=map.get(key);t.subjects.add(s.mapel);t.classes.add(s.kelas)});return[...map.values()].sort((x,y)=>x.name.localeCompare(y.name,'id'))}async function loadTeachers(){setState('Menyiapkan direktori Guru Pondok…');await loadSchedule();const list=teacherDirectory();stat([[list.length,'Guru Pondok'],[new Set(state.schedule.map(x=>x.mapel)).size,'Mata pelajaran'],[state.schedule.length,'Alokasi jadwal']]);$('content').innerHTML='<div class="me-empty"><b>KPI Pekan Kerja Personel</b><br>Nilai Senin–Sabtu siang dibaca otomatis. Manajer mengisi observasi pembelajaran dan keteladanan.<br><a class="me-btn primary" href="../pendidikan/kpi-guru.html?v=3">Buka KPI Pekan Kerja Guru</a></div>'+(list.length?list.map(t=>rowCard(t.name,`${esc([...t.subjects].join(', '))}<br>${esc([...t.classes].join(' • '))}`,status('Kepondokan','info'),`<button class="me-btn" data-open-feature="coaching" data-menu="menu-pembinaan-guru" data-teacher="${esc(t.name)}">Pembinaan</button><button class="me-btn" data-open-feature="observation" data-menu="menu-observasi-pembelajaran" data-teacher="${esc(t.name)}">Observasi</button>`)).join(''):'<div class="me-empty">Belum ada Guru Pondok pada jadwal unit ini.</div>');hideState()}
...
function openSheet(type,data={}){
  ...
  const forms={
    ...
    ,revision:{title:data.reportRequestPath?'Teruskan Instruksi Revisi':'Buka Revisi Nilai',eyebrow:data.reportRequestPath?'PERMINTAAN SUPERVISOR':'SESI NILAI FINAL',html:`<p class="me-empty">${esc(data.guru)} • ${esc(data.mapel)} • ${esc(data.kelas)}<br>${esc(data.jenis)} • ${esc(data.periode)} • ${esc(data.tahun)} • Final</p>`+(data.supervisorNote?`<p><b>Catatan Supervisor:</b><br>${esc(data.supervisorNote)}</p>`:'')+field('reason',data.reportRequestPath?'Instruksi Manajer kepada Guru':'Alasan Membuka Revisi','textarea',data.reason||'', 'required placeholder="Contoh: Periksa dan perbaiki nilai Bahasa Arab sesuai catatan Supervisor."')+'<p>Seluruh nilai yang sudah tersimpan tetap dipertahankan. Guru pemilik sesi dapat memperbaiki nilai lalu melakukan finalisasi kembali.</p>'}
  };
  ...
  $('saveBtn').textContent=type==='revision'?(data.reportRequestPath?'Teruskan ke Guru & Buka Revisi':'Buka Revisi Nilai'):'Simpan';
  ...
  form.dataset.reportRequestPath=data.reportRequestPath||'';
...
async function saveAction(e){
  ...
  try{
    if(type==='revision'){
      const reportRequest=state.reportRequests.get(e.currentTarget.dataset.reportRequestPath)||null;
      await openScoreRevision(id,String(v.reason||'').trim(),reportRequest);
      toast(reportRequest?'Instruksi diteruskan ke Guru dan sesi dibuka untuk revisi.':'Sesi nilai dibuka untuk revisi.');
...
async function openScoreRevision(key,reason,reportRequest=null){
  assert();if(!reason)throw new Error('Alasan membuka revisi wajib diisi.');
  if(reportRequest&&(session.activeRole!=='MANAJER'||!R.isEducationManager(a)))throw new Error('Hanya Manajer Pendidikan yang dapat meneruskan permintaan Supervisor kepada Guru.');
  ...
  const [markerSnap,indexSnap,requestSnap]=await Promise.all([get(ref(db,`${PATH.final}/${key}`)),get(ref(db,`${PATH.scoreIndex}/${key}`)),reportRequest?get(ref(db,reportRequest._path)):Promise.resolve(null)]);
  ...
  if(reportRequest){const live=requestSnap?.val();if(!live||live.status!=='NEEDS_REVISION'||String(live.managerRequest?.requestedAt||live.reviewedAt||'')!==String(reportRequest.managerRequest?.requestedAt||reportRequest.reviewedAt||''))throw new Error('Permintaan Supervisor telah berubah. Segarkan rekap.');}
  ...
  if(reportRequest){
    const requestBase=reportRequest._path;
    updates[`${requestBase}/managerRequest/status`]='FORWARDED_TO_TEACHER';
    updates[`${requestBase}/managerRequest/note`]=reportRequest.managerRequest?.note||reportRequest.reviewNote||'';
    updates[`${requestBase}/managerRequest/forwardedAt`]=at;
    updates[`${requestBase}/managerRequest/forwardedByUid`]=identity.id;
    updates[`${requestBase}/managerRequest/forwardedByName`]=identity.name;
    updates[`${requestBase}/managerRequest/instruction`]=reason;
    updates[`${requestBase}/managerRequest/scoreSessionKey`]=key;
    updates[`${requestBase}/managerRequest/teacherName`]=marker.guru||'';
    updates[`${requestBase}/managerRequest/subject`]=marker.mapel||'';
    updates[`${requestBase}/updatedAt`]=at;
    updates[`${requestBase}/history/${Date.now()}`]={status:'FORWARDED_TO_TEACHER',at,byUid:identity.id,byName:identity.name,note:reason,scoreSessionKey:key,teacherName:marker.guru||'',subject:marker.mapel||''};
  }
...
document.addEventListener('click',event=>{
  const button=event.target.closest('[data-revise-score],[data-forward-report]');if(!button)return;
  const sessionKey=button.dataset.scoreSession||button.dataset.reviseScore,sessionData=state.scoreSessions.get(sessionKey),marker=sessionData?.marker;if(!marker)return;
  const request=button.dataset.forwardReport?state.reportRequests.get(button.dataset.forwardReport):null;
  const supervisorNote=request?.managerRequest?.note||request?.reviewNote||'';
  openSheet('revision',{id:sessionKey,guru:marker.guru,mapel:marker.mapel,kelas:sessionClasses(marker).join(' + '),jenis:marker.jenisUjian,periode:marker.periode,tahun:marker.tahunAkademik,reportRequestPath:request?._path||'',supervisorNote,reason:supervisorNote?`Mohon periksa dan perbaiki ${marker.mapel} sesuai catatan Supervisor: ${supervisorNote}`:''});
});
function currentAcademicYear(){const d=new Date(),year=d.getFullYear(),start=d.getMonth()>=6?year:year-1;return`${start}/${start+1}`}
function populateScorePeriods(){const type=$('scoreType').value,periods=type==='triwulan'?['Triwulan 1','Triwulan 2','Triwulan 3','Triwulan 4']:type==='semester'?['Semester 1','Semester 2']:window.CahayaAcademicInputProgress.MONTHS,current=$('scorePeriod').value;$('scorePeriod').innerHTML=periods.map(item=>`<option ${item===current?'selected':''}>${item}</option>`).join('');if(type==='bulanan'&&!current)$('scorePeriod').value=window.CahayaAcademicInputProgress.MONTHS[new Date().getMonth()]}
async function setup(){assert();const c=config[view]||config.control;$('eyebrow').textContent=c[0];$('pageTitle').textContent=c[1];$('pageSubtitle').textContent=c[2];$('identityChip').textContent=identity.name;$('unitChip').textContent=unitLabel;$('scopeChip').textContent=`${unitLabel} • Kepondokan`;const today=jakartaDate(),r=range('MONTH');$('selectedDate').value=today;$('startDate').value=r.start;$('endDate').value=r.end;$('scoreYear').innerHTML=`<option>${currentAcademicYear()}</option>`;populateScorePeriods();const daily=['control','material'].includes(view),monthly=view==='scores',ranged=['followup','findings','coaching','observation','kpi'].includes(view);$('dateTools').hidden=!daily;$('monthTools').hidden=!monthly;$('rangeTools').hidden=!ranged;if(view==='guide'){$('dateTools').hidden=$('monthTools').hidden=$('rangeTools').hidden=true}await loadCurrent();const teacher=params.get('guru')||'';if(teacher&&view==='coaching')openSheet('coach',{guru:teacher});if(teacher&&view==='observation')openSheet('observe',{guru:teacher});if(params.get('createFinding')==='1')openSheet('finding',{tanggal:params.get('tanggal')||today,sourceId:params.get('sourceId')||'',sourceType:params.get('sourceType')||'MANUAL',sourcePath:params.get('sourcePath')||'',personil:params.get('personil')||'',subject:params.get('subject')||'',summary:params.get('summary')||''})}
function refreshCurrent(){state.schedule=[];sharedCache?.clear(`${cacheKey}:schedule:v2`);return loadCurrent()}
document.addEventListener('click',e=>{const d=e.target.closest('[data-date]'),r=e.target.closest('[data-range]'),nf=e.target.closest('[data-new-finding]'),follow=e.target.closest('[data-follow]'),coach=e.target.closest('[data-coach]'),observe=e.target.closest('[data-observe]'),feature=e.target.closest('[data-open-feature]');if(d){$('selectedDate').value=jakartaDate();loadCurrent()}if(r){const x=range(r.dataset.range);$('startDate').value=x.start;$('endDate').value=x.end;loadCurrent()}if(nf)openSheet('finding',{tanggal:nf.dataset.sourceDate,sourceId:nf.dataset.newFinding,sourceType:nf.dataset.source,sourcePath:nf.dataset.sourcePath,personil:nf.dataset.personnel,subject:nf.dataset.subject,summary:nf.dataset.summary});if(follow)openSheet('follow',state.findings.find(x=>x.id===follow.dataset.follow)||{});if(coach)openSheet('coach',{guru:coach.dataset.coach});if(observe)openSheet('observe',{guru:observe.dataset.observe});if(feature)openManagerFeature(feature.dataset.openFeature,feature.dataset.teacher,feature.dataset.menu)});$('closeSheet').onclick=()=>$('sheet').classList.remove('open');$('actionForm').onsubmit=saveAction;$('reloadBtn').onclick=refreshCurrent;$('reloadMonthBtn').onclick=refreshCurrent;$('scoreType').onchange=()=>{populateScorePeriods();loadCurrent()};['selectedDate','scoreYear','scorePeriod','startDate','endDate'].forEach(id=>$(id).onchange=loadCurrent);$('backBtn').onclick=()=>parent!==window&&typeof parent.openAuthorizedMenu==='function'?parent.openAuthorizedMenu('menu-home'):history.back();addEventListener('cahaya:suspend',()=>{state.suspended=true;state.schedule=[];state.rows=[];state.findings=[]});document.addEventListener('visibilitychange',()=>{if(document.hidden)state.suspended=true;else if(state.suspended){state.suspended=false;refreshCurrent()}});setup().catch(error=>{console.error('Gagal memuat workspace Manajer Pendidikan',error);const message=String(error?.message||'').startsWith('Jadwal Kepondokan')?error.message:'Data belum dapat dimuat. Coba Segarkan.';setState(message);toast(message)});
