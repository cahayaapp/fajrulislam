...
const config={control:['KONTROL OPERASIONAL','Kontrol Hari Ini','Pantau jadwal, kehadiran Guru, pelaksanaan KBM, pembukaan, finalisasi, kelas kosong, dan isu hari ini.'],material:['CAPAIAN MATERI','Capaian Materi','Bandingkan target dan laporan capaian materi Guru Pondok.'],followup:['TINDAK LANJUT','Tindak Lanjut','Kerjakan temuan aktif dari seluruh sumber operasional pendidikan.'],teachers:['PERSONIL PENDIDIKAN','Guru Pondok','Direktori dan konteks operasional Guru Pondok dalam unit Anda.'],scores:['KONTROL NILAI & REVISI','Nilai Ujian','Pantau nilai dan teruskan permintaan perbaikan Supervisor kepada Guru pengampu.'],observation:['OBSERVASI OPERASIONAL','Observasi Pembelajaran','Catat kekuatan dan area perbaikan pelaksanaan pembelajaran.'],findings:['RIWAYAT OPERASIONAL','Riwayat Temuan','Tinjau temuan yang sudah selesai berdasarkan kalender.'],coaching:['PEMBINAAN PERSONIL','Pembinaan Guru','Catat apresiasi, coaching, briefing ulang, atau target perbaikan.'],kpi:['BUKTI OPERASIONAL','KPI Manajer Pendidikan','Bukti kerja pendidikan pada periode terpilih; bukan skor atau peringkat.'],guide:['PANDUAN KERJA','Panduan Manajer Pendidikan','Batas scope, siklus kontrol, dan jalur eskalasi operasional.']};
...
async function loadScores(){
  setState('Memuat nilai dan permintaan perbaikan dalam scope…');await loadSchedule();const month=$('selectedMonth').value;let scores=[];
  ...
  await loadFinalScoreSessions(month);await loadReportCorrectionRequests();hideState()
}
async function loadFinalScoreSessions(month){
  ...
  for(const [key,marker] of Object.entries(markers)){
    ...
    if(!records.some(item=>String(item.data.tanggal_ujian||'').slice(0,7)===month))continue;
