...
const ATTENDANCE_LOCATION_DEFAULTS={PUTRA:{id:'pondok_putra',nama:'Pondok Putra',latitude:-2.7583058,longitude:107.7125462,radiusMeter:700},PUTRI:{id:'pondok_putri',nama:'Pondok Putri',latitude:-2.7458405,longitude:107.6472273,radiusMeter:700}};
let attendanceLocationsPromise=null;
async function attendanceLocations(){if(!attendanceLocationsPromise)attendanceLocationsPromise=get(ref(db,'cahaya_app/pengaturan_absensi_guru/lokasi')).then(s=>s.val()||{}).catch(()=>({}));return attendanceLocationsPromise}
function scheduleUnit(r){const t=K.norm([r.unit,r.kelas,r.rombel,r.kelas_kelompok].filter(Boolean).join(' '));return t.includes('putri')?'PUTRI':'PUTRA'}
function distanceMeters(a,b,c,d){const R=6371000,p=x=>x*Math.PI/180,dLat=p(c-a),dLon=p(d-b),q=Math.sin(dLat/2)**2+Math.cos(p(a))*Math.cos(p(c))*Math.sin(dLon/2)**2;return 2*R*Math.atan2(Math.sqrt(q),Math.sqrt(1-q))}
function devicePosition(){return new Promise((resolve,reject)=>{if(!navigator.geolocation)return reject(new Error('GPS tidak tersedia pada perangkat ini.'));navigator.geolocation.getCurrentPosition(resolve,e=>reject(new Error(e.code===1?'Izin lokasi diperlukan untuk absensi guru.':'Lokasi belum dapat dibaca. Aktifkan GPS lalu coba lagi.')),{enableHighAccuracy:true,timeout:15000,maximumAge:15000})})}
async function verifyTeacherLocation(r){const unit=scheduleUnit(r),saved=await attendanceLocations(),raw=Object.values(saved||{}).find(x=>x&&x.aktif!==false&&K.norm(x.unit||x.nama).includes(unit.toLowerCase())),zone={...ATTENDANCE_LOCATION_DEFAULTS[unit],...(raw||{})},pos=await devicePosition(),latitude=Number(pos.coords.latitude),longitude=Number(pos.coords.longitude),accuracy=Math.round(Number(pos.coords.accuracy)||0),distance=distanceMeters(latitude,longitude,Number(zone.latitude),Number(zone.longitude)),radius=Number(zone.radiusMeter)||700,tolerance=Math.min(Math.max(accuracy,35),350),effective=Math.max(0,distance-tolerance-60);if(effective>radius)throw new Error(`Anda berada di luar zona ${zone.nama||unit}. Jarak ${Math.round(distance)} m, radius ${radius} m.`);return{latitude,longitude,akurasiMeter:accuracy,jarakMeter:Math.round(distance),jarakEfektifMeter:Math.round(effective),radiusMaksimalMeter:radius,lokasiId:zone.id||unit.toLowerCase(),lokasiNama:zone.nama||unit,unit}}
function showDirectAttendanceResult(type,r,record,message){
  const ok=type==='success';
  $('teacherSheetIcon').className='sheet-head-icon '+(ok?'attendance':'discipline');
  $('teacherSheetTitle').textContent=ok?'Absensi Berhasil Disimpan':'Absensi Gagal';
  $('teacherSheetSub').textContent=message||'';
  $('teacherSheetStats').innerHTML=stat(record?.waktuTibaWIB||'—','Waktu')+stat(ok?'Tersimpan':'Gagal','Status',ok?'success':'danger')+stat(record?.keterlambatanMenit||0,'Menit Terlambat',record?.keterlambatanMenit>0?'danger':'success');
  $('teacherSheetListTitle').textContent=ok?'Data Kehadiran':'Keterangan';$('teacherSheetCount').textContent='';
  $('teacherSheetList').innerHTML=ok
    ? `<div class="sheet-row"><span class="sheet-avatar success">✓</span><span class="sheet-row-copy"><b>${esc(subjectText(r))}</b><small>${esc(classText(r))} • ${esc(timeText(r))}</small></span><span class="sheet-badge">${esc(record.statusKehadiranLabel||'Tersimpan')}</span></div><button class="sheet-row-action" style="width:100%;margin-top:8px;min-height:38px" onclick="openScheduleModal()">Kembali ke Jadwal Hari Ini</button>`
    : `<div class="sheet-empty">${esc(message||'Data absensi belum berhasil disimpan. Silakan coba lagi.')}</div><button class="sheet-row-action" style="width:100%;margin-top:8px;min-height:38px" onclick="openScheduleModal()">Kembali</button>`;
  showSheet()
}
window.openAutoAttendance=async i=>{
  const r=TODAY_SCHEDULE[i];if(!r)return;
  const date=K.jakartaNow().date,existing=matchAttendance(r,date);
  if(existing){showDirectAttendanceResult('success',r,existing,'Kehadiran untuk jadwal ini sudah tercatat.');return}
  const btn=document.querySelector(`#teacherSheet .sheet-row:nth-child(${i+1}) .sheet-row-action`);
  ...
  try{
    ...
    if(!record){
      ...
      const clockNow=jakartaClock(),st=directAttendanceStatus(r),teacherCode=String(IDENT.code||r.guruKode||r.kodeGuru||'').trim().toUpperCase();
      record={idAbsensi:`${teacherKey}-${jadwalId}`,tanggal:date,waktuTibaWIB:clockNow,guruUid:USER.uid||'',guruKey:teacherKey,guruKodeKalender:teacherCode,namaGuru:teacherName,emailGuru:USER.email||'',jadwalId,hariJadwal:r.hariLabel||r.hari||'',jadwalMulai:r.jamMulai||'',jadwalSelesai:r.jamSelesai||'',jadwalLabel:timeText(r),mapel:subjectText(r),kelas:classText(r),nomorKelas:r.nomorKelas||'',unit:r.unit||verifiedLocation.unit,metodeAbsensi:'GPS_OTOMATIS',statusKehadiran:st.status,statusKehadiranLabel:st.label,keterlambatanMenit:st.late,sumber:'ABSENSI_GURU_GPS_LANGSUNG',validasiKelas:'JADWAL_DAN_RADIUS_TERVALIDASI',validasiRadiusDiberlakukan:true,...verifiedLocation,versi:'2026/2027'};
      const logRef=push(ref(db,'cahaya_app/log_absensi_guru')),updates={};
      updates[path]={...record,waktuTibaServer:serverTimestamp(),dibuatPada:serverTimestamp()};
      updates[`cahaya_app/log_absensi_guru/${logRef.key}`]={...record,idLog:logRef.key,waktuTibaServer:serverTimestamp(),dibuatPada:serverTimestamp()};
    ...
    console.error('Absensi langsung gagal',e);
