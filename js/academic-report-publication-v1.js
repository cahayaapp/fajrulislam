...
(function(root,factory){
  ...
  function readiness({allRecords=[],studentRecords=[],type,year,period,student,applicable}){
    ...
    const expected=[...new Set(cohort.map(subject).map(norm).filter(Boolean))];
    ...
    const completed=[...new Set(finals.map(subject).map(norm).filter(Boolean))];
    const missing=expected.filter(item=>!completed.includes(item));
  ...
  function renderScoreCards(records,{formatTeacher=value=>value}={}){
    return (records||[]).map(record=>{const score=ScorePolicy?.reportScore?ScorePolicy.reportScore(record):Number(record.nilai_total),remedial=ScorePolicy?.isRemedial?ScorePolicy.isRemedial(record):record.is_remedial===true,note=String(record.keterangan||'').trim();return `<article class="period-report-card"><div class="period-report-head"><div><h4>${escape(subject(record))}</h4><div class="period-report-meta">${escape(formatTeacher(record.guru_penguji||record.guru||'Guru'))}</div></div><div class="period-report-score">${Number.isFinite(score)?score:'—'}</div></div><div class="period-report-components">${components(record)}</div>${remedial?'<span class="report-remedial">Remedial</span>':''}${note?`<div class="period-report-note">${remedial?'Remedial · ':''}${escape(note)}</div>`:''}</article>`}).join('');
  }
  return {TYPES,STATUS,PUBLICATION_REQUIRED_FROM,norm,key,stableStudentKey,path,isPublished,isLegacyMonthlyPublished,statusLabel,effectiveStatus,subject,studentKeyOf,classOf,finalRecords,readiness,renderScoreCards};
});
