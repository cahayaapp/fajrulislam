...
(() => {
  ...
  const descriptions={
    ...
    'menu-dashboard-operasional':'Catat pelaksanaan tugas dapur','menu-dashboard-operasional--checklist':'Pastikan area dapur bersih dan siap','menu-dapur-stock':'Pantau bahan dan ketersediaan stok','menu-dapur-menu':'Atur menu dan jumlah porsi santri','menu-dapur-received':'Catat bahan masuk per area','menu-dapur-usage':'Catat penggunaan bahan per area',
  };
  ...
  async function init(){
    ...
    if(!wali&&!window.CahayaRoleMenuV2){await new Promise((resolve,reject)=>{const script=document.createElement('script');script.src=new URL('js/role-menu-v2.js?v=272',root);script.onload=resolve;script.onerror=reject;document.head.append(script)})}
    ...
    if(role==='DAPUR'){
      const desc={'menu-dashboard-operasional':'Catat pelaksanaan tugas dapur','menu-dashboard-operasional--checklist':'Pastikan area dapur bersih dan siap','menu-dapur-stock':'Pantau bahan dan ketersediaan stok','menu-dapur-menu':'Atur menu dan jumlah porsi santri'};
      quick=quick.map((item,index)=>({...item,description:desc[item.id],icon:['report','check','chart','book'][index]}));
    }
    if(wali){
      const waliQuick={
