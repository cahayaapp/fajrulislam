...
(async()=>{
 ...
 try{
  ...
  for(const text of ['Fiqih','Guru Uji','85','Baik','Lisan','Tulisan','Tashnif','Praktek'])assert(screen.toLowerCase().includes(text.toLowerCase()),text);
  ...
  assert.deepEqual(data.headers,['Mata Pelajaran','Guru','Lisan','Tulisan','Tashnif','Praktek','Nilai','Predikat']);
  assert.equal(data.rows.length,8);assert(!data.rows.join(' ').includes('QA_MATERIAL'));assert.equal(data.width,190);
  ...
  console.log('PASS: actual screen/monthly PDF/bulk-print renderers; eight PDF columns; scores/components/predikat retained; populated material fixture unchanged.');
