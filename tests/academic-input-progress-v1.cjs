...
test('Supervisor review merges every scheduled subject with report records',()=>{const html=fs.readFileSync(path.join(root,'supervisor/laporan-akademik.html'),'utf8');for(const text of ['Review Rapor','Progres Input Nilai','expectedSubjects(student)','P.reportRows(x.ready.records,x.expected)','cahaya_app/jadwal_pelajaran'])assert(html.includes(text),text)});
